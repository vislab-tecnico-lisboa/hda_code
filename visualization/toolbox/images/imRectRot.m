...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function intitialize( varargin )
        ...
        [hParent,ellipse,rotate,pos,lims,showLims,color,colorc,lw,ls,cross]=...
            getPrmDflt(varargin,dfs,1);%sets default values of parameters
        if(numel(pos)==4), %if only 4 elementes in pos, probably adding rotation angle
            pos=[pos(:)' 0];
        end
        if(numel(lims)==4),
            lims=[lims(:)' 0];
        ...
        if( ellipse ),
            ...
            linePrp={'color',color,'LineWidth',lw,'LineStyle',ls};
        end
        if(isempty(pos));
            vis='off';
        else
        ...
        for i=1:4+cross,
            hBnds(i)=line(linePrp{:},'Visible',vis);
        end
        ...
        if( rotate )
            ...
            for i=1:4,
                hCntr(i)=rectangle(circPrp{:},'Visible',vis);
            end
    ...
    function setPos( posNew, ignoreLims )
        if(isempty(hBnds) || isempty(hPatch));
            return;
        end; 
        L=lims;
        ...
        if( ~isempty(L) && (nargin<2 || ~ignoreLims) )
            if( posNew(5)==lims(5) )
                ...
                [pc,rs,R]=rectInfo(L); 
                p0=-rs; p1=rs;
                p=(rectToCorners(posNew)-pc(ones(4,1),:))*R; xs=p(:,1); 
                ys=p(:,2);
        ...
        pos=posNew;
        [pts,xs,ys]=rectToCorners(pos);
        vert=[xs ys ones(4,1)];
        face=1:4;
        set(hPatch,'Faces',face,'Vertices',vert);
        % draw ellipse
        if(~isempty(hEll)),
            [pc,rs]=rectInfo(posNew);
            th=pos(5);
        ...
        for i=1:length(hBnds),
            ...
            set(hBnds(i),'Xdata',xs(ids),'Ydata',ys(ids)); % Draws each line
            % of the rectangle, exactly where pos indicates
            if(rotate && i<=4),
                x=mean(xs(ids));
                y=mean(ys(ids));
