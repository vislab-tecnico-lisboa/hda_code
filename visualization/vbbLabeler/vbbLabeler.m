function vbbLabeler_hda_Commented_Dario( objTypes, vidNm, annNm )
% Video bound box (vbb) Labeler. Version 0.1 @ ObservIt by Fabio
%
...
nStep     = 25;   % number of objects to display in lower panel
repLen    = 1;    % number of seconds to replay on left replay
maxCache  = 300;  % max cache length (set as high as memory allows)
...
enStr = {'off','on'};
[hFig, pLf, pRt, pMenu, pObj, pSeq ] = deal([]); %setting everything to []
[A,skip,offset,curInd,seqH,objApi,dispApi,filesApi] = deal([]);
[fVid, sr] = deal([]); % setting this to global so I can access the video file name -Dario

% initialize all
makeLayout();
% filesApi: functions related to the video file and annotation file (open/close/save)
...
if(~isempty(annNm)), filesApi.openAnn(annNm); end
    ...
    function makeLayout()
        ...
        axsPr=[units {'Parent'}]; o4=[1 1 1 1]; clr=[.1 .1 .1];%color
        ...
        icn=load('vbbIcons_HDA'); icn=icn.icn; % button pictures
        %icn=load('vbbIcons'); icn=icn.icons; % button pictures
        
        % hFig: main figure
        set(0,units{:}); ss=get(0,'ScreenSize');
        if( ss(3)<800 || ss(4)<600 ), error('screen too small'); end
        pos = [(ss(3)-780)/2 (ss(4)-580)/2 780 580]; %show up centered
        % Name: title; NumberTitle off: don't show 'Figure #'; 
        % Menubar none: no 'File Edit ...' bar
        % Color: background color
        % Visible off: starts invisble
        hFig = figure( 'Name','VBB Labeler', 'NumberTitle','off', ...
            'Toolbar','auto', 'MenuBar','none', 'ResizeFcn',@figResized, ...
            'Color','k', 'Visible','on', 'DeleteFcn',@exitLb, ps,pos, ...
...
%             'keyPressFcn',@keyPress );
        ...
        pObj.hObjTp = uic( pObj.h,'Style','popupmenu',fs,8,units{:},...
            ps,[31 2 100 25],st,objTypes,'Value',1); %popup menu for the target names
        ...
        pObj.hBtRecord = uic(pObj.h,btnPr{:},[345 3 25 25],'CData',icn.record); %added extract images button
        pObj.hCn = [pObj.hObjTp pObj.hBtNew pObj.hBtDel ...
            pObj.hBtPrv pObj.hBtNxt pObj.hCbFix pObj.hBtRecord];
        ...
        function keyPress( h, evnt ) %#ok<INUSL> Added the dependency to 
            % setFrame and objToggle to the respective Api structers
            ...
            %previous & next frame (z and c)
            if( char==99 ), objApi.objToggle(+1); end
            if( char==122 ), objApi.objToggle(-1); end
            
            %+x frames (x def. by the ui) (a and d)
            if( char==97 ), dispApi.setFrame('-'); end
            if( char==100 ), dispApi.setFrame('+'); end
            
            %+-offset in right gui frame(q and e)
            %if( char==113 ), dispApi.setPlay(-1); end
            %if( char==101 ), dispApi.setPlay(+1); end
            if( char==113 ),
                offs = get(pRt.slOffst,'Value');
                if(offs > 1),
                    dispApi.setOffset(offs - 1);
                end;
            end
            if( char==101 ),
                offs = get(pRt.slOffst,'Value');
    ...
    %% function objApi = objMakeApi    
        ...
        set(pObj.hBtRecord, 'callback', @(h,e) cropImages());
        ...
        % create api \\Added the: 'objToggle'
...
%         function 
        ...
        function init()
            [objId objS objE objInd seqObjs lims] = deal([]);
            objId=-1;
            objSelect(-1); %[objId objS objE objInd]=deal(-1);
            isAnn=~isempty(A); 
            prepPlay();
            if( ~isAnn ), 
                return;
            ...
            t='*new-type*';
            objTypes=[setdiff(objTypes,t) t];
            set( pObj.hObjTp, 'String', objTypes, 'Value',1 );
        end
        
        function prepPlay()
            if(objId>0),
                A=vbb('setRng',A,objId,objS+1,objE+1);
            end
            set(pObj.hStSiz,'String','');
            set(pObj.hCn,'Enable','off');
            apiRng.enable(0);
            apiOcc.enable(0);
            apiLck.enable(0);
        end
        ...
        function seqI = prepSeq()
            seqI=100*ones(seqH,seqH*nStep,3,'uint8'); 
            seqObjs=[];
            if(isempty(A)), 
                return; 
            end; 
            ...
            objSe=min(objS,curInd); 
            objEe=max(objE,lstInd);
            ...
            rng(s:e)=1;
            for i=1:n
                ind0=curInd+(i-1)*skip;
                ind1=objGrpInd(ind0,1);
        ...
        function hs = drawRects( flag, ind )
            ...
            function hs = drawRect(pos,posv,lims,lbl,static,id,ind,sid)
                if(colType),%defined as always 0 (line44)
                    t=find(strcmp(lbl,objTypes)); %t= index of label in
                    %objtypes matrix
                else 
                    t=id;
                end
                col=rectCols(mod(t-1,size(rectCols,1))+1,:); %choose color 
                %depending on label, one for each
                if(id~=objId),% ls defines line style for the rectangles
                    ls='-';  
                else
                    if(ind==objInd),
                        ls='--'; 
                ...
                %defines parameters for the function imRectRot
                [hs,api]=imRectRot('pos',pos,'lims',lims,rp{:});%creates object rectangle alligned with axis(rotate=0)
            ...
            function objSetPos( pos, id, ind, sid )
                if(sid>=0),
                    o=seqObjs(sid+1);
                    pos=o.pos0-(o.pos-pos)/o.res;
                end
                if(sid>0),
                    dispApi.setOffset(sid);
                end
                pos=constrainPos(pos);
                A=vbb('setVal',A,id,'pos',pos,ind+1);
                if(objId==id), 
                    objS=min(objS,ind); 
                    objE=max(objE,ind); 
                end
                objSelect(id,ind); 
                ind0=curInd+floor((ind-curInd)/skip)*skip;
                ind1=objGrpInd(ind0,0); 
                lks=zeros(ind1-ind0+1,1);
        ...
        function objDel()
            ...
            %% A seems to be the struct associated with the Annotation file
            % A.objInit, A.objLbl, A.objStr, A.objEnd, A.objHide are "vbb"'s
            % vbb (video bounding box): Data per object (indexed by integer id) includes the following fields:
            ...
            %
            % A.objLists are "bb"'s (bounding box), 1 struct per frame 
...
%             targetDir = 'extractedImages';
%             vbb( 'vbbToFiles', A, targetDir)
%         end
        ...
        function objSelect( id, ind )
            ...
            if(id==-1),
                [objId objS objE objInd]=deal(-1); 
                return;
            end
            objS=vbb('getVal',A,id,'str')-1; 
            objE=vbb('getVal',A,id,'end')-1;
            objId=id; objInd=ind;
        end
    ...
    function api = dispMakeApi()
        ...
        api = struct( 'requestUpdate',@requestUpdate, 'init',@init, ...
            ...
            'setFrame', @setFrame, ... % \\Added the setFrame to the structure -Fabio
            'cropImages', @cropImages); % -Dario
        ...
        function init( sr1 )
            ...
            if(isstruct(sr)),
                ...
                info=struct('numFrames',0,'width',0,'height',0,'fps',25);
            end
            if(fps),
                info.fps=fps;
            end
        ...
        function dispLoop()
            ...
            while( 1 )
                ...
                k=k+1;
                if(0 && ~needUpdate), % DEBUG verbose line
                    fprintf('%i draw events.\n',k);
                end
                if( ~needUpdate ),
                    looping=0;
                    return;
        ...
        function updateDisp()
            ...
            % possibly switch display modes % WHICH DISPLAY MODES ARE THERE?  
        ...
        function requestUpdate( clearHs )
            if(nargin==0 || isempty(clearHs)), 
                clearHs=0; 
            end
            if(clearHs), 
                [hImLf, hImRt, hImPl, hImSeq]=deal([]);
            end
            needUpdate=true;
            dispLoop();
        end
    ...
    %% function api = filesMakeApi()
    % fVid=[PATHSTR,NAME], usually d=./ and f=VideoName.seq
    
        % variables
        [fVid, fAnn, tSave, tSave1] = deal([]);
        
        % callbacks on the top menu bar (Video/Annotation)
        ...
        function updateMenus()
            m=pMenu;
            en=enStr{~isempty(fVid)+1};% if Video is not available then 
            %annotation New and open menu is set to off and vice versa
            nm='VBB Labeler';
            set([m.hVidCls m.hAnnNew m.hAnnOpn],'Enable',en);
            en=enStr{~isempty(fAnn)+1};% if annotation file is not 
            %available then annotation Save and close is set to off and
            %vice versa
            set([m.hAnnSav m.hAnnCls],'Enable',en);
            if(~isempty(fVid)), [d,nm1]=fileparts(fVid);
                nm=[nm ' - ' nm1];
            end
            set(hFig,'Name',nm); 
            objApi.init();
            dispApi.requestUpdate(); %draws the bounding boxes
        end
        ...
        function openVid( f )
            if( nargin>0 )
                [d f]=fileparts(f); %[PATHSTR,NAME,EXT] = fileparts(FILE)
                if(isempty(d)), d='.'; 
                end;
                d=[d '/']; f=[f '.seq'];
            else
                if(isempty(fVid)), d='.'; 
                else d=fileparts(fVid); 
                end
                [f,d]=uigetfile('*.seq','Select video',[d '/*.seq']); %Standard open file dialog box.
            end
            if( f==0 ),
                return;
            end;
            ...
            try
                s=0; 
                srLocal=seqIo(fVid,'r',maxCache);
                s=1;
                dispApi.init(srLocal); 
                updateMenus();
            catch er
        ...
        function closeAnn()
        %% Close Annotation
        % if not empty, ask if wanna save
        ...
        function openAnn( f )
            ...
            try
                if(~exist(fAnn,'file'))% fills the structure A
            ...
            tSave=clock(); 
            tSave1=tSave; 
            updateMenus(); % turns the options 
            %from the annotation menu from grey to black and writes the
            %bounding boxes.
        ...
        function newAnn()
            ...
            fAnn=[fVid(1:end-3) 'vbb'];
            assert(~isempty(fVid));
            A=vbb('init',dispApi.nFrame());
            updateMenus();
        end
        
        function backupAnn()
            if(isempty(tSave) || etime(clock,tSave)<30), 
                return; 
            end
            A1=objApi.annForSave();
            if(isempty(A1)), 
                return;
            end
            tSave=clock();
            timeStmp=etime(tSave,tSave1)>60*5;
            if(timeStmp), 
                tSave1=tSave; 
                fAnn1=fAnn; 
            else
                fAnn1=[fAnn(1:end-4) '-autobackup' fAnn(end-3:end)];
            end
    ...
    function cropImages()
        % fVid,
        % sr,
        % display('Proof that global variables works.')
        % return,
         
        % Extract camera number from video file path
        %   First make it consistent both in windows and linux (make the
        %   '\' be '/')
        ind=strfind(fVid,'\');
        fVid_consistent = fVid;
        fVid_consistent(ind) = '/';
        ...
        hwait = waitbar(0,'Cropping ALL images..');
        for i=1:length(A.objLists) % For each frame
            
            % i=240 % DEBUG
            if(~isempty(A.objLists{i}))%if the frame contains at least one object
                % Proof that seek works :D
                % sr.seek(40); figure(42); imshow(sr.getframe());
                
                sr.seek(i-1); % Seek aparently starts at zero
                I=sr.getframe();
                
                
                
                for j=1:size(A.objLists{i},2) % For each detection in the frame
                    personID=A.objLists{i}(j).id;
                    personLabel=A.objLbl{personID};
                    occluded = A.objLists{i}(j).occl;
                    pos = round(A.objLists{i}(j).pos);
                    posTop = pos(2);
                    posW = pos(3);
                    posLeft = pos(1);
                    posH = pos(4);
                    % Ensuring crop is inside image boundaries
                    posTop = max(1,posTop);
                    posLeft = max(1,posLeft);
                    posBottom = min(size(I,1),posTop+posH);
                    posRight = min(size(I,2),posLeft+posW);
                    crop=I(posTop:posBottom, posLeft:posRight, :);
                    % Debug show cropped images
                    if occluded,
                        % titlestring = ['Person ' num2str(personID) ' occluded'];
                        namefile = [folderName '/' personLabel ...
                            'frame' int2str(i) 'camera' int2str(cam) 'OCC.png'];
                    else
                        % Debug show images
                        % figure(42), imshow(I),
                        ...
                        % titlestring = ['Person ' num2str(personID)];
                        namefile = [folderName '/' personLabel ...
                            'frame' int2str(i) 'camera' int2str(cam) '.png'];
                        % figure(16), imshow(crop), title(titlestring), pause(0.1),
                    end
                    imwrite(crop,namefile);
                end
            end
            waitbar(i/length(A.objLists),hwait); pause(0.01),
        end
        close(hwait); pause(0.01), % pause because Matlab is stupid and seems to need it to update the bar
    end
...
api = struct('delete',@delete1, 'enable',@enable, 'setPos',@setPos, ...
    ...
    function [lf rt] = getBnds( rng1 )
        if(nargin==0 || isempty(rng1));
            rng1=rng;
        end
        [v,lf]=max(rng1);
        [v,rt]=max(fliplr(rng1));
        rt=n-rt+1;
        if(v==0); 
            lf=-1;
            rt=-1;
