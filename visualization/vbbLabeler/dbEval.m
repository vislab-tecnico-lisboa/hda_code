...
exps = {
    'Reasonable',     [50 inf],  [.65 inf], 0,   .5,  1.25             % 1: 50, at least 65% visible
    'All',            [20 inf],  [.2 inf],  0,   .5,  1.25
    'Scale=large',    [100 inf], [inf inf], 0,   .5,  1.25             % 3: scale = far
    'Scale=near',     [80 inf],  [inf inf], 0,   .5,  1.25
    'Scale=medium',   [30 80],   [inf inf], 0,   .5,  1.25
    'Scale=far',      [20 30],   [inf inf], 0,   .5,  1.25             % 6: scale = far
    'Occ=none',       [50 inf],  [inf inf], 0,   .5,  1.25             % 7:  50, no occlusion
...
bnds = [-inf -inf +inf +inf];     % discard bbs outside this pixel range WHY? WHAT THE HECK... MATTEO
...
for d=1:length(dataNames), dataName=dataNames{d};
    ...
    warning('Make sure that maximum frame number read in dts and gts works correctly.');
...
for g=1:nGt
    for d=1:nDt
        ...
        %%MATTEO I SHOULD ENABLE THIS AGAIN WARNING WARNING WARNING WARNING
        %WARNING WARNING WARNING WARNING
        fprintf('\tExp %i/%i, Alg %i/%i: %s/%s\n',g,nGt,d,nDt,stre,stra);
        hr = exps(g).hr.*[1/exps(g).filter exps(g).filter];
        for f=1:n
            %         if(f==164)
            %             fprintf('I got to the right image\n');
            %         end
            bb=dt{f};
            dt{f}=bb(bb(:,4)>=hr(1) & bb(:,4)<hr(2),:);
        end
...
for p=1:nPlots
    ...
    if( plotAlg )
        ...
        kp=[find(strcmp(stra,'VJ')) find(strcmp(stra,'HOG')) 1 1]; %MATTEO: What the heck is this?
        [~,ord]=sort(scores(p,:));
        
        %MATTEO: with this part enabled, the script will only plot two curves
        %(the best?)
        %     kp=ord==kp(1)|ord==kp(2);
        %
        %     j=find(cumsum(~kp)>=plotNum-2);
        %     kp(1:j(1))=1;
        %     ord=fliplr(ord(kp));
    ...
    %MATTEO, I don't want the figure to close.
    %close(1);
    pause; %MATTEO: for multiple plots
end


%MATTEO if you want to save gtsthe scores, break the execution here and save
%them
end
...
for g=1:nGt
    for d=1:nDt
        % augment each bb wigtsth set/video/frame index and flatten
...
%  varargin - prm struct or nam  dts = loadDt( algs, plotName, aspectRatio );
...
for i=1:nExp
    ...
    for s=1:length(setIds)
        ...
        % WE DON'T FILTER THE BB'S LIKE DOLLAR USED TO DO
        %             A = loadVbb(s,v);
        ...
        for f=frames, bb=bbs(bbs(:,1)==f+1,2:6);
            bb=bbApply('resize',bb,1,0,aspectRatio);
            k=k+1;
            gt{k}=bb;
        end
...
for i=1:nAlg
    ...
    %WE LOAD THE DATA BY HAND. IT SHOULD WORK FOR MORE SETS, BUT ONLY ONE VIDEO PER SET.
    %THE DATA IS IN:
    %../../../hda_detections/AcfInria/camera60/Detections/allD.txt
