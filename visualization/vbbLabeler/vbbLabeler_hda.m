function vbbLabeler_hda( objTypes, vidNm, annNm )
...
% allow personal configurations
if exist('vbbLabeler_hda_mycnf.m', 'file')
    mycnf= vbbLabeler_hda_mycnf;
    if ischar(mycnf), eval(mycnf);
    else for ind=1:length(mycnf), eval(mycnf{ind}); end
    end
end
...
if(~isempty(annNm)), filesApi.openAnn(annNm); end
  ...
  function makeLayout()
    ...
    function keyPress( h, evnt ) %#ok<INUSL> Added the dependency to setFrame and objToggle to the respective Api structers
  ...
  function api = objMakeApi()
    ...
    api=struct( 'init',@init, 'drawRects',@drawRects, ...
      ...
      'annForSave',@annForSave, 'annSaved',@annSaved, 'objToggle', @objToggle);
    ...
    function hs = drawRects( flag, ind )
      ...
      switch flag
        case {'panelLf','panelRt'}
          ...
          if(n>0), [d,ord]=sort([os.id]==objId); os=os(ord); end
  ...
  function api = dispMakeApi()
    ...
    % create api \\Added the setFrame to the structure
    ...
    function requestUpdate( clearHs )
      ...
      if(clearHs), [hImLf, hImRt, hImPl, hImSeq]=deal([]); end
    ...
    function I=getImg(f)
      sr.seek(f); I=sr.getframe(); if(ndims(I)==2), I=I(:,:,[1 1 1]); end
    end
...
api = struct('delete',@delete1, 'enable',@enable, 'setPos',@setPos, ...
  ...
  function [lf rt] = getBnds( rng1 )
    ...
    [v,lf]=max(rng1); [v,rt]=max(fliplr(rng1)); rt=n-rt+1;
