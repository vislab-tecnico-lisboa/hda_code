...
%   vbb( 'vbbToFiles', A, tarDir, [frameFileNames] )
...
A = orderfields(A,order); % order the fields of A in alphabetic order 
A.altered = false;
end
...
function vbbToFiles( A, tarDir, frameFileNames )
% export single frame annotations to tarDir/*.txt
nFrm=A.nFrame;
% fName=@(f) ['I' int2str2(f-1,5) '.txt']; %fancy but does not save space
                                           % and is illegible
if(nargin<3 || isempty(frameFileNames)),
    for f=1:nFrm,
        frameFileNames{f} = ['I' int2str2(f-1,5) '.txt'];
        % frameFileNames{f}=fName(f);
    end;
end
...
for f=1:nFrm
    ...
    for j=1:nObj
        o=A.objLists{f}(j); 
        objs(j).lbl=A.objLbl{o.id}; 
        objs(j).occ=o.occl;
        objs(j).bb=round(o.pos); 
        objs(j).bbv=round(o.posv);
    end
    bbGt('bbSave',objs,[tarDir '/' frameFileNames{f}]);
end
end
