function dist1toAll = MSCRmatch_1toall(Blob,trainingBlobs)
% 
...
for i = 1%:length(dataf)
	...
	dist_y_n	= cell(length(Blobs),1);
	dist_color_n= cell(length(Blobs),1);
	...
	for j = 1:length(dataf)
		...
		if i == j
            ...
            % Distance between all blobs of i with all blobs of j
			for k=1:max_info
				for h=1:min_info
