function [allGMatToSave, allDplusGTplustBB] = GTandDetMatcher_oneCamera(mode,camera)
...
%     for camera = testCameras
%         fprintf('gtAndDetMatcher: Working on camera: %d\n',camera);
        
        reIdsDirectory         = [experimentDataDirectory sprintf('/camera%02d', camera) '/ReIds_' reIdentifierName];
        reIdsAndGtDirectory    = [experimentDataDirectory sprintf('/camera%02d', camera) '/ReIdsAndGT_' reIdentifierName];
...
%         fclose(fid);
        ...
        %Read GT file (VBB)
        GTName = [hdaRootDirectory '/hda_annotations' sprintf('/cam%02d.txt',camera)];
        GTMat = vbb('vbbLoadTxt',GTName);
        
        if strcmp(mode,'normal')
            reIdsMat = dlmread([reIdsDirectory '/allR.txt']);
        elseif strcmp(mode,'detections')
            localDetectionsDirectory = [thisDetectorDetectionsDirectory sprintf('/camera%02d',camera) '/Detections'];
            reIdsMat = dlmread([localDetectionsDirectory '/allD.txt']);
        end
        nReIds=size(reIdsMat,1);
        %Work on one detectition file at a time
        %wbr = waitbar(0, ['gtAndDetMatcher on camera ' int2str(camera) ', image 0/' int2str(nReIds)]);
        ...
        for image=1:nReIds
            ...
            %    waitbar(image/nReIds, wbr, ['GT Matching on cam ' int2str(camera) ', img ' int2str(image) '/' int2str(nReIds)]);
            %end
