...
%       - pmskset4_128x64.mat: body-part mask file 
    ...
    if exist([trainingSetPath '/allT.txt'],'file')
        ...
        % Assume that want to use all detections from the training cameras as
        % trainign data
        % TODO: make for here for all trainingCameras
         [~, TrainMat] = GTandDetMatcher_oneCamera('detections',trainCameras);
    end
    ...
    if isempty(nameList)
        ...
        numOfSamples = size(nameList,1);
        assert(size(TrainMat,1) == size(nameList,1), 'This was assumed to be true, if it is not, re-think the code'),
    end
    ...
    masks = load_pre_computed_body_part_masks(trainingSetPath,numOfSamples);
    ...
    if loadImages && exist(allTrainingDataStructure_path,'file') && ~keepImages
        ...
        for i=1:size(TrainMat,1)
            ...
            camera = trainSample(1);
            frame = trainSample(2); 
            allTrainingDataStructure(i).camera      = camera;
            allTrainingDataStructure(i).frame       = frame;
            ...
            if loadImages
                ...
                if ~isempty(nameList)
                    ...
                    bb = trainSample(5:8);
                    image = getFrameAndCrop(camera, frame, bb);
                end
